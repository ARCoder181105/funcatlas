...
function Count({ functions }: { functions: number }) {
  return (
    <span className="font-mono text-[11px] tabular-nums text-ink-muted">{functions}</span>
  );
}
  ...
  return (
    ...
    >
      <div className="grid gap-10 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] lg:items-center">
        <ul className="space-y-5 text-sm leading-relaxed text-ink-muted">
          <li>
            <span className="text-ink">One file card at a time</span>, with as many function
            branches off it as you open. Collapsing a branch hides its subtree without forgetting
            it.
          </li>
          <li>
            <span className="text-ink">⌘K jumps to any function by name</span>, across every file in
            the repository, and lands the canvas on it.
          </li>
          <li>
            <span className="text-ink">The selection survives a reload</span> — repository, file and
            every branch you opened. Rebuilding a map by hand after a refresh is not exploring.
          </li>
        </ul>

        <Bezel innerClassName="overflow-hidden p-2">
          <Files className="w-full" defaultOpen={["apps", "services"]}>
              ...
              <FolderPanel>
                <SubFiles defaultOpen={["web"]}>
                  <FolderItem value="web">
                    ...
                    <FolderPanel>
                      <FileItem icon={FileCode2} className="font-mono" meta={<Count functions={6} />}>
                        App.tsx
                      </FileItem>
                      <FileItem icon={FileCode2} className="font-mono" meta={<Count functions={11} />}>
                        Canvas.tsx
                      </FileItem>
                      <FileItem icon={FileCode2} className="font-mono" meta={<Count functions={9} />}>
                        graph.ts
                      </FileItem>
                    </FolderPanel>
                  </FolderItem>
                  ...
                  <FolderItem value="api">
                    ...
                    <FolderPanel>
                      <FileItem icon={FileCode2} className="font-mono" meta={<Count functions={7} />}>
                        routes.ts
                      </FileItem>
                    </FolderPanel>
                  </FolderItem>
            ...
            <FolderItem value="services">
              ...
              <FolderPanel>
                <SubFiles>
                  <FileItem icon={FileCode2} className="font-mono" meta={<Count functions={23} />}>
                    resolver.go
                  </FileItem>
                  <FileItem icon={FileCode2} className="font-mono" meta={<Count functions={17} />}>
                    extract.go
                  </FileItem>
                </SubFiles>
              </FolderPanel>
