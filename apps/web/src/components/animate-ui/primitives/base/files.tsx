  ...
  return (
    <FilesProvider value={{ open: openValue ?? [] }}>
      <Accordion
        ...
        defaultValue={defaultOpen}
        value={open}
        // Narrowed rather than cast: this Accordion is `multiple`, so Base UI
        // always hands back an array, but its type is `AccordionValue<unknown>`
        // in the version this project locks.
        onValueChange={(next) => setOpenValue(Array.isArray(next) ? next.map(String) : [])}
        style={{
          position: 'relative',
          overflow: 'auto',
          ...style,
