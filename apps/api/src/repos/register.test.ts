...
describe("runParser", () => {
  it("resolves when the parser exits 0", async () => {
    ...
    await expect(runParser("https://github.com/owner/repo", bin)).resolves.toBeUndefined();
  });
  ...
  it("passes the URL as an argument, not through a shell", async () => {
    ...
    const bin = stubParser("echo-args", `printf '%s' "$2" > "${path.join(dir, "seen")}"`);
  ...
  it("reports a non-zero exit with the tail of stderr", async () => {
    const bin = stubParser("fail", 'echo "clone failed: repository not found" >&2\nexit 1');

    await expect(runParser("https://github.com/owner/repo", bin)).rejects.toThrow(ParseError);
    await expect(runParser("https://github.com/owner/repo", bin)).rejects.toMatchObject({
      timedOut: false,
  ...
  it("kills a parser that hangs, rather than holding the request open", async () => {
    ...
    await expect(runParser("https://github.com/owner/repo", bin, 200)).rejects.toMatchObject({
      timedOut: true,
    });
  });
  ...
  it("reports a missing binary rather than throwing something unhandled", async () => {
    await expect(
      runParser("https://github.com/owner/repo", path.join(dir, "does-not-exist")),
